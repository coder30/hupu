    ...
    render() {
        ...
        return(
            ...
            <ScrollView style={{marginBottom: 30}}>
                <ImageBackground resizeMethod={'scale'} source={{uri: this.state.dataSource.img_m.slice(0,this.state.dataSource.img_m.indexOf('?'))}} imageStyle={{opacity:1}} style={{width:Dimensions.get("window").width,  height: Dimensions.get("window").width*202/360, justifyContent:'center', alignItems:'center', backgroundColor: '#E5E5E5'}}>
                    ...
                    <View>
                        ...
                        <FlatList 
                            ...
                            <TouchableOpacity style={{flexDirection:'row', paddingTop:10, paddingBottom:10, marginBottom:4}} onPress={() =>  {item.type==1?navigation.navigate('News', { nid: item.nid, replies: item.replies}): navigation.navigate('Details', { name:'湿乎乎的话题',fid: 1048, tid: item.link.slice(19,27)})}}>
                                <Image source={{uri:item.img.slice(0,item.img.indexOf('?'))||item.thumbs[0]}} style={{width:90, height:70, backgroundColor: '#E5E5E5'}}/>
