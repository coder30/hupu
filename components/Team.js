...
export default class Team extends React.Component {
  ...
  _contentViewScroll(e){
    ...
    else if(offsetY<1400){
      this.setState({
        tab: 1
      })
    }
    else {
      var i = Math.floor((offsetY-1475)/250+2);
      this.setState({
        tab: i
      })
        ...
        renderItem={({item, index}) =>
            <View>
              ...
              <View style={{flexDirection: 'row', backgroundColor: 'rgba(0, 0, 0, 0.12)', padding: 7}}>
                <Text style={{paddingRight: 70, fontSize: 12}}>{item.name}</Text>
              ...
              :<View style={{flexDirection: 'row', justifyContent: 'space-between', backgroundColor: 'rgba(0, 0, 0, 0.12)', padding: 7}}>
                ...
                <Text style={{fontSize:12, paddingRight: 10}}>数据</Text>
              </View>
              }
                <FlatList
                data={item.data}
